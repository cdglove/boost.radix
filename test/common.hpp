...
inline std::vector<bits_type> generate_random_bytes(
    std::size_t num_bytes, int max_value = 255) {
...
inline void set_bits_lsb(
  ...
  boost::uint16_t current_value = bytes[byte_offset];
  ...
  boost::uint16_t mask    = (~(boost::uint16_t(~0) << num_bits)) << bit_in_byte;
  boost::uint16_t value16 = boost::uint16_t(value) << bit_in_byte;
...
inline void set_bits_msb(
  ...
  boost::uint16_t current_value = bytes[byte_offset] << 8;

  // This is only to guiard against reading off the end of the array
  if(bit_in_byte + num_bits > 8)
    current_value |= boost::uint16_t(bytes[byte_offset + 1]);

  boost::uint16_t mask = (~(boost::uint16_t(~0) << num_bits))
                       << (16 - num_bits - bit_in_byte);
  boost::uint16_t value16 = boost::uint16_t(value) << (16 - num_bits - bit_in_byte);
