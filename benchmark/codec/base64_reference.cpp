...
static void Base64_Encode_Reference(benchmark::State& state)
{
...
BENCHMARK(Base64_Encode_Reference)
    ->Arg(128)
    ->Arg(1024)
    ->Arg(8 * 1024)
    ->Arg(64 * 1024)
...
static void Base64_Decode_Reference(benchmark::State& state)
{
    std::vector<bits_type> data = generate_random_bytes(state.range(0));
    std::string encoded =
        base64::encode(std::addressof(*data.begin()), data.size());
...
BENCHMARK(Base64_Decode_Reference)
    ->Arg(128)
    ->Arg(1024)
    ->Arg(8 * 1024)
    ->Arg(64 * 1024)
